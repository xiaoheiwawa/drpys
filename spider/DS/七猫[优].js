...
async function getQmParamsAndSign() {
    try {
        ...
        for (let c of base64Str) {
            qmParams += globalThis.char_map[c] || c; 
        }
        console.log('[调试] qm-params结果:', qmParams); 
        // 2.5 签名参数拼接：
        let paramsStr = `AUTHORIZATION=app-version=10001application-id=com.duoduo.readchannel=unknownis-white=net-env=5platform=androidqm-params=${qmParams}reg=${globalThis.keys}`;
...
var rule = {
    ...
    let signString = `extend=page=${page}read_preference=0track_id=${trackId}wd=${KEY}${globalThis.keys}`;
    let sign = md5(signString);
    console.log('[调试] 搜索接口sign:', sign);
    let encodedKey = encodeURIComponent(KEY);
    let url = `${globalThis.h_ost}/api/v1/playlet/search?extend=&page=${page}&wd=${encodedKey}&read_preference=0&track_id=${trackId}&sign=${sign}`;
